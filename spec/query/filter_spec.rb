...
RSpec.describe Query::Filter do
  describe '#call' do
    ...
    subject { described_class.new(Car.all, params).call }

    context 'with no params' do
      let(:params) { {} }
      it 'make' do
        expect(described_class.new(Car.all, params).call).to contain_exactly(car1, car2, car3)
      end
    end
    ...
    context 'when make filter is applied' do
      ...
      it 'make' do
        expect(subject).to contain_exactly(car1, car3)
      end
    end
    ...
    context 'when model filter is applied' do
      ...
      it 'model' do
        expect(subject).to contain_exactly(car2)
      end
    end
